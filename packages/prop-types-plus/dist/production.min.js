...
var allOfType = ((propTypes$$1 = []) => {
  const propType = (...rest) => {
    const failedPropType = propTypes$$1.find(p => p(...rest));
    return failedPropType === null || failedPropType === void 0 ? void 0 : failedPropType(...rest);
  };

  propType.isRequired = (...rest) => {
    const failedPropType = [propTypes.any.isRequired].concat(propTypes$$1).find(p => p(...rest));
    return failedPropType === null || failedPropType === void 0 ? void 0 : failedPropType(...rest);
  };
...
var func$1 = convert_1('isString', isString_1, _falseOptions);

func$1.placeholder = placeholder;
var isString$1 = func$1;

var withRequired = (propType => {
  if (process.env.NODE_ENV !== 'production') {
    if (!isFunction$1(propType)) throw new Error('PropTypesPlus (withRequired): propType argument must be a function');
  }
...
const propTypeIsRequiredIf = (predicate, propType) => (props, ...rest) => {
  if (process.env.NODE_ENV !== 'production') {
    if (!isFunction$1(propType)) throw new Error('PropTypesPlus (isRequiredIf): propType argument must be a function');
    if (!isFunction$1(predicate) && !isString$1(predicate)) throw new Error('PropTypes (isRequiredIf): predicate argument must be of type function or string');
  }

  return (isString$1(predicate) ? props[predicate] : predicate(props)) ? propType.isRequired(props, ...rest) : propType(props, ...rest);
};
...
const propTypeIsRequiredIfNot = (predicate, propType) => (props, ...rest) => {
  if (process.env.NODE_ENV !== 'production') {
    if (!isFunction$1(propType)) throw new Error('PropTypesPlus (isRequiredIfNot): propType argument must be a function');
    if (!isFunction$1(predicate) && !isString$1(predicate)) throw new Error('PropTypes (isRequiredIfNot): predicate argument must be of type function or string');
  }

  return (isString$1(predicate) ? props[predicate] : predicate(props)) ? propType(props, ...rest) : propType.isRequired(props, ...rest);
};
...
const propTypeNotRequiredIf = (predicate, propType) => (props, ...rest) => {
  if (process.env.NODE_ENV !== 'production') {
    if (!isFunction$1(propType)) throw new Error('PropTypesPlus (notRequiredIf): propType argument must be a function');
    if (!isFunction$1(predicate) && !isString$1(predicate)) throw new Error('PropTypes (notRequiredIf): predicate argument must be of type function or string');
  }

  return (isString$1(predicate) ? props[predicate] : predicate(props)) ? isUndefined(props, ...rest) : propType(props, ...rest);
};
